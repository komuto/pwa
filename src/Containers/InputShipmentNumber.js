...
class InputShipmentNumber extends React.Component {
  ...
  render () {
    ...
    const { notification, tabs, processingOrderDetail, receiptNumber } = this.state
...
const ShipmentReceiptNumber = (props) => {
  const { processingOrderDetail, receiptNumber } = props
          ...
          <article className='media'>
            <div className='media-left'>
              <figure className='image user-pict'>
                <img src='../images/icon-info-blue.svg' alt='pict' />
              </figure>
            </div>
            <div className='media-content'>
              <div className='content'>
                <p>
                  <strong>Anda memiliki waktu 3 hari sampai tanggal 17 Oktober 2017 untuk menginformasikan nomor resi. Jika tidak kami akan membatalkan pesanan ini</strong>
                </p>
              </div>
            </div>
          </article>
      ...
      <section className='section is-paddingless'>
        <div className='container is-fluid'>
          <a className={`button is-primary is-large is-fullwidth js-option ${props.submiting && 'is-loading'}`}
            onClick={() => props.submit()}>Kirim Pesan ke Seller</a>
        </div>
      </section>
    </div>
  )
                      ...
                      <div className='right-top'>
                        {
                          ...
                          ? <a className='button is-primary is-outlined' onClick={() => Router.push(`/send-message?id=${processingOrderDetail.orderDetail.invoice.id}&type=reseller`)}>Kirim Pesan</a>
                          : <a className='button is-primary is-outlined' onClick={() => Router.push(`/send-message?id=${processingOrderDetail.orderDetail.invoice.id}&type=buyer`)}>Kirim Pesan</a>
                        }
                      </div>
                    </div>
                  </div>
